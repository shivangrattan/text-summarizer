...
input_method = st.radio("Select a method to input data: ", ("Upload Data", "Enter Manually"))
...
def create_word_embeddings():
    word_embeddings = {}
    with open('glove6b100dtxt/glove.6B.100d.txt') as f:
        for line in f:
            values = line.split()
            word = values[0]
...
# TF-IDF
sentences = sent_tokenize(text, preproc)
...
st.write("")
st.write("")
sn = int(st.number_input("Enter Number of Sentences for GloVe Word Embedding: ", min_value=0))
